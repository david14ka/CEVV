...
import static com.davidkazad.cevv.utils.Constants.TAG;
...
public class Favoris extends Model {
    ...
    public Favoris(Page mPage) {
        fid = mPage.getBookId() + "" + mPage.getPid();
        songId = mPage.getPid();
    ...
    public static boolean exists(Page mPage) {
        return new Select().from(Favoris.class).where("bookId=? AND songId=?", mPage.getBookId(), mPage.getPid()).exists();
    }
